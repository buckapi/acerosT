...
import { Butler } from './butler.services'; 
...
export class DataApiService {
	...
	//	const token = this.AuthRESTService.getToken();
		const url_api=`https://db.corpcssca.com:9010/api/products/${id}/?access_token$={token}`;
		return this.http
	...
	deleteCar(id: string){
	//	const token = this.AuthRESTService.getToken();
		const url_api=`https://db.corpcssca.com:9010/api/cars/${id}/?access_token$={token}`;
		return this.http
	...
	//	const token = this.AuthRESTService.getToken();
		const url_api=`https://db.corpcssca.com:9010/api/chats/${id}/?access_token$={token}`;
		return this.http
	...
	//	const token = this.AuthRESTService.getToken();
		const url_api=`https://db.corpcssca.com:9010/api/messages?filter[where][idChat]=${idChat}/?access_token$={token}`;
		return this.http
	...
	//	const token = this.AuthRESTService.getToken();
		const url_api=`https://db.corpcssca.com:9010/api/cards/${id}/?access_token$={token}`;
		return this.http
		.delete<MemberInterface>(url_api, {headers: this.headers})
		.pipe(map(data => data));
	}
	getTransationByBranch(branch: string){
		const url_api = `https://db.corpcssca.com:9010/api/transactions?filter[where][idBranch]=${branch}`;
	...
	getAllCars(){
		const url_api = 'https://db.corpcssca.com:9010/api/cars';
		return this.http.get(url_api);
	}

	getAllChats(){
		const url_api = 'https://db.corpcssca.com:9010/api/chats';
		return this.http.get(url_api);
	}
	getMessagesBy(idChat: string){
		const url_api = `https://db.corpcssca.com:9010/api/messages?filter[where][idChat]=${idChat}`;
		this.messages = this.http.get(url_api);
		return ( this.http.get(url_api));		
	}
	
	getAllCards(){
		const url_api = 'https://db.corpcssca.com:9010/api/cards';
		return this.http.get(url_api);
	}

	getAllOrders(){
		const url_api = 'https://db.corpcssca.com:9010/api/orders';
		return this.http.get(url_api);
	}
	getAllTransactions(){
		const url_api = 'https://db.corpcssca.com:9010/api/transactions';
		return this.http.get(url_api);
	}
	getProduct(id: string){
		const url_api = `https://db.corpcssca.com:9010/api/products/${id}`;
		return this.http.get(url_api);
	}

	getCierresByBranch(branch: string){
		const url_api = `https://db.corpcssca.com:9010/api/infos?filter[where][idBranch]=${branch}`;
	...
	carUpdate(car :CarInterface, id: string){
		...
		const url_api=`https://db.corpcssca.com:9010/api/cars/${id}`;
		return this.http
	...
	cardUpdate(card :CardInterface, id: string){
		// let token = this.authService.getToken();
		const url_api=`https://db.corpcssca.com:9010/api/cards/${id}`;
		return this.http
	...
	getPartsById(userId: string){
		const url_api = `https://db.corpcssca.com:9010/api/products?filter[where][userId]=${userId}`;
		this.parts = this.http.get(url_api);
		return ( this.http.get(url_api));		
	}
	getCarsById(userId: string){
		const url_api = `https://db.corpcssca.com:9010/api/cars?filter[where][userId]=${userId}`;
		this.cars = this.http.get(url_api);
		return ( this.http.get(url_api));		
	}
	getCardByUserId(userId: string){
		const url_api = `https://db.corpcssca.com:9010/api/cards?filter[where][userd]=${userId}`;
		this.cards = this.http.get(url_api);
		return ( this.http.get(url_api));		
	}
	getSerialT(branch: string){
		const url_api = `https://db.corpcssca.com:9010/api/branchs/${branch}`;
	...
	saveMessage(message:MessageInterface){
		const url_api='https://db.corpcssca.com:9010/api/messages';
		return this.http
